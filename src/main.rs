...
struct Order {
    takeaway: bool,
    items: std::collections::HashMap<Dish, u32>, //it does have hash and eq derived
    total: u32,
}
...
impl Order {
    ...
    fn add_dish(&mut self, dish: Dish) {
        self.items.entry(dish).or_insert(0);
        if let Some(x) = self.items.get_mut(&dish) {
            *x += 1;
        }
        self.total += Dish::price(&dish);
    }
